<div id="top" class="menu-superior-general">
	<div >
		<img src="img/Colegio/0. Escudo.jpg" style="float: left; max-width: 8%; margin-left: 15%; margin-top: 1%">
		<label class="label-menu-superior-general">INSTITUTO<br><br>SANTA TERESITA</label>
		<img src="img/Colegio/3. Foto esquina.jpg" style="float: left; max-width: 30%; margin-left: 7%; margin-top: -3%">
	</div>
	<div>
		<div class="buttons-menu-container">
			<a class="btn btn-primary" onclick="app.home.openLogin(true)"><i class="icon-user icon-white"></i></a>
		</div>
		<div id="login-menu-container" class="login-menu-container">
			<input type="text" class="nombre-usuario-singin" placeholder="Nombre de Usuario">
			<input type="password" class="nombre-usuario-singin" placeholder="Contraseña">
			<input type="submit" class="btn btn-danger" value="Cancel" onclick="app.home.openLogin(false)">
			<input type="submit" class="btn btn-primary" value="Login" onclick="app.home.openLogin(false)">

		</div>
	</div>	
</div>




